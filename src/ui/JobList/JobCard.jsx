...
const JobCard = ({ job, onViewJob }) => {
  ...
  return (
    <div className="rounded-lg shadow-sm  relative pr-4" style={{
  boxShadow: "0 18px 20px -10px rgba(54, 113, 209, 0.6)",
      ...
      {/* Desktop Layout (md+) */}
  <div className="hidden md:grid grid-cols-6  gap-x-8">
  {/* Logo */}
  <div className="col-span-1 row-span-2">
    <img
      src={logo}
      alt={`${company} logo`}
      className="rounded-lg object-cover  max-h-40  translate-y-7 translate-x-4"
    />
  </div>

  {/* Title + Location */}
  <div className="col-span-4 lg:mt-8">
  ...
  {/* Button + Salary */}
 <div className="col-start-5 col-span-2 row-start-2 flex flex-col h-full items-end text-gray-700 font-bold">
  <p className="text-sm  ">
    {salaryRange} <span className="text-yellow-600 font-bold"> /{compensationperiod}</span>
  </p>
  <button
    onClick={handleViewJob}
    className="text-blue-600 font-normal hover:underline mb-2 hover:text-blue-800"
  >
    Browse Job
  </button>
</div>
  ...
  {/* Job Type Badge */}
  <span className="absolute top-8 right-2 rounded font-medium bg-blue-500  text-white text-sm px-2 py-1">
    {jobType}
  </span>
</div>
